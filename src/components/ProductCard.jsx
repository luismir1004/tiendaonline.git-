...
const ProductCard = memo(({ product }) => {
  ...
  const handleMouseEnter = () => {
    ...
    // (Optional: Implement query prefetch here)
    
    // Prefetch variant images
    if (product.variants) {
        product.variants.forEach(v => {
            const img = new Image();
            img.src = v.image; // Browser cache warmup
        });
    }
  };
  ...
  return (
    ...
    >
        ...
        <div className="relative aspect-[4/5] bg-slate-50 overflow-hidden">
            ...
            {/* Main Image with Cross-fade support via SmartImage key prop if needed, 
                but for instant switch standard img might be faster or SmartImage with cache */}
            <SmartImage 
                src={displayImage} 
                alt={product.name}
